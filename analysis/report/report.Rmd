...
begin in Section 1.1 with a review of the relevant literature
and then describe the simulation study and its results in
Sections 2 and 3.
...
Despite the theoretical optimality results, limited
simulation-based evidence exists on how different visit
schedules perform in finite samples under realistic data-
generating mechanisms. In this report, we present a Monte
Carlo study comparing two five-visit designs for a 100-patient,
two-arm longitudinal trial with linearly divergent treatment
effects:
...
power of `r sprintf("%.1f%%", 100 * power_equal)`, compared
with `r sprintf("%.1f%%", 100 * power_clust)` for the
clustered design. The empirical standard error of
$\hat{\beta}_3$ is `r sprintf("%.4f", se_equal)` under equal
spacing and `r sprintf("%.4f", se_clust)` under the clustered
design.
...
The results are, in general, broadly consistent with the
theoretical findings of @winkensOptimalTimepointsClinical2005.
When the data-generating mechanism includes both random slopes
and independent residual error -- a structure that combines
features of compound symmetry and AR(1) with measurement error
-- the clustered design, which concentrates observations near
the boundaries of the study period, appears to yield different
precision for the treatment-by-time interaction compared with
equal spacing.
