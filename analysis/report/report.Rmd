...
abstract: |
  ...
  configuration, and a template (`R/simulation.R`) for extending the
...
are compared. `R/simulation.R` in the accompanying compendium
...
kable(
  ...
  booktabs = TRUE,
  digits = 4,
  caption = "Exact GLS standard errors of the treatment-by-time
...
kable(
  ...
  caption = "Analytic (closed-form, no simulation) relative
    efficiency of the clustered versus equal-spacing design
    across covariance structures and random-slope variance."
) |>
  kable_styling(
    latex_options = "hold_position",
    font_size = 9
...
kable(
  ...
  caption = paste(
    ...
    "computed following @morris2019simulation."
  )
) |>
  kable_styling(
...
%.5f)", se_equal, mcse_se_equal)` under equal spacing and `r
sprintf("%.4f (MCSE %.5f)", se_clust, mcse_se_clust)` under the
clustered design. The paired-bootstrap relative efficiency (ratio
...
bootstrap interval `r sprintf("%.2f", re_boot$ci[1])`-`r
sprintf("%.2f", re_boot$ci[2])`), consistent with the analytic
...
we considered only a linear treatment effect;
@winkens2007polynomial showed that optimal designs differ for
...
D-optimal framework of @ouwens2002maximin and the Bayesian
approach of @abebe2014bayesian provide principled starting
