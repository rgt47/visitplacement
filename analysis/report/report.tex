...
\renewcommand{\stamptime}{2026-08-20 13:58 PDT}
\renewcommand{\stampversion}{\detokenize{7f3a5e9-wip}}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{booktabs}
...
\begin{table}[!h]
\centering
\caption{\label{tab:analytic-efficiency}Exact GLS standard errors of the treatment-by-time
...
\begin{table}[!h]
\centering
\caption{\label{tab:scenario-grid}Analytic (closed-form, no simulation) relative
...
error (MCSE) computed following\textsuperscript{\citeproc{ref-morris2019simulation}{\textbf{morris2019simulation?}}}: MCSE of the
...
\begin{table}[!h]
\centering
\caption{\label{tab:summary-table}Simulation results for the treatment-by-time interaction, with Monte Carlo standard errors (MCSE) of each metric computed following @morris2019simulation.}
\centering
...
0.00106) under equal spacing and \texttt{r\ sprintf("\%.4f\ (MCSE\ \%.5f)",\ se\_clust,\ mcse\_se\_clust)} under the
...
bootstrap interval 1.06-\texttt{r\ sprintf("\%.2f",\ re\_boot\$ci{[}2{]})}), consistent with the analytic
